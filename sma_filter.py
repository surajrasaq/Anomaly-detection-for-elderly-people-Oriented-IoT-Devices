...
arm_swing = pd.read_csv('./data/raw_data/walk_Home2Tram_Station.txt', delimiter = ',',  header= None, names = COLUMNS)[:2000]
#arm_vibration = pd.read_csv('./data/cane_data/vibrations.txt',  delimiter = ',',  header= None, names = COLUMNS)[:5000]
...
def temm(act):
    ...
    return  sma
#
no_filter = temm(arm_swing)
filters = filter(arm_swing)
img = [no_filter, filters]
titles = ['Irregular Arm Swinging (Signal with noise)','Irregular Arm Swinging (Signal after filter) ' ]
